...
func (sess *session) Write(p []byte) (n int, err error) {
	if sess.pty != nil {
		m := len(p)
		// normalize \n to \r\n when pty is accepted and ONLCR mode is set.
		// ONLCR (opcode 72 per RFC 4254 Section 8) translates output
		// newline to carriage-return + newline. Default is enabled (1)
		// when not explicitly disabled (0), matching typical PTY behavior.
		const opOnlcr = 72 // RFC 4254 terminal mode opcode for ONLCR
		onlcr := true
		if v, ok := sess.pty.Modes[opOnlcr]; ok && v == 0 {
			onlcr = false
		}
		if onlcr {
